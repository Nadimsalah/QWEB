...
$res = mysqli_query($con,"SELECT *, (6372.797 * acos(cos(radians($UserLat)) * cos(radians(ShopLat)) * cos(radians(ShopLongt) - radians($UserLongt)) + sin(radians($UserLat)) * sin(radians(ShopLat)))) AS distance FROM Shops JOIN Following ON Shops.ShopID= Following.ShopID WHERE Shops.HasStory='YES' AND Following.UserID = '$UserID' AND Shops.Status = 'ACTIVE'  HAVING distance <= 50 ORDER BY priority DESC , distance ASC");
