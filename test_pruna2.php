...
$payload = json_encode([
    "input" => [
        "prompt" => "Virtual try-on: Take the clothing from the SECOND image and put it on the person in the FIRST image. The output MUST be the SAME person from the FIRST image, with the same face, same hair, same pose, and same background. ONLY change the upper-body clothing to match the SECOND image perfectly. DO NOT generate a new person.",
        "images" => [$u, $p],
...
echo "Submit: " . json_encode($data) . "\n";

if (!empty($data['generation_url'])) {
    $genUrl = $data['generation_url'];
    echo "Downloading generation_url: $genUrl\n";
    ...
    $origUser = file_get_contents($userUrl);
    echo "Original User size: " . strlen($origUser) . " bytes\n";
    $origProd = file_get_contents($prodUrl);
    echo "Original Prod size: " . strlen($origProd) . " bytes\n";
}
?>
