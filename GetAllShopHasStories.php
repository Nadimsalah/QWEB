...
$res = mysqli_query($con,"SELECT *, (6372.797 * acos(cos(radians($UserLat)) * cos(radians(ShopLat)) * cos(radians(ShopLongt) - radians($UserLongt)) + sin(radians($UserLat)) * sin(radians(ShopLat)))) AS distance FROM Shops JOIN ShopStory ON ShopStory.ShopID = Shops.ShopID WHERE HasStory='YES' AND Shops.Status = 'ACTIVE' AND ShopStory.StoryStatus = 'ACTIVE' GROUP BY Shops.ShopID HAVING distance <= 50  ORDER BY priority DESC , distance ASC ");
