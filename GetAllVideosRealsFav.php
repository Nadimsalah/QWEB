...
FROM (
    SELECT 
        ...
        Shops.ShopOpen,
        (6372.797 * ACOS(
            COS(RADIANS($UserLat)) * COS(RADIANS(Shops.ShopLat)) *
            COS(RADIANS(Shops.ShopLongt) - RADIANS($UserLongt)) +
            SIN(RADIANS($UserLat)) * SIN(RADIANS(Shops.ShopLat))
        )) AS distance,
        Posts.PostID AS SortID
    FROM Shops
    JOIN Posts ON Shops.ShopID = Posts.ShopID
    JOIN Likes ON Likes.UserID = $UserID
        AND REPLACE(Likes.PostID,'r','') = Posts.PostID
    ...
    SELECT 
        ...
        ShopStory.StotyID AS SortID
    FROM Shops
    JOIN ShopStory ON Shops.ShopID = ShopStory.ShopID
    JOIN Likes ON Likes.UserID = $UserID
        AND REPLACE(Likes.PostID,'r','') = ShopStory.StotyID
    WHERE Shops.Status = 'ACTIVE'
      AND ShopStory.StoryStatus = 'ACTIVE'
      AND ShopStory.StotyType = 'Video'
) AS all_data
HAVING distance <= 10050
