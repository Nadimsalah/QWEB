...
if($UserLongt!=""){
    ...
    $res = mysqli_query($con,"SELECT *, (6372.797 * acos(cos(radians($UserLat)) * cos(radians(ShopLat)) * cos(radians(ShopLongt) - radians($UserLongt)) + sin(radians($UserLat)) * sin(radians(ShopLat)))) AS distance FROM Shops JOIN Categories ON Categories.CategoryId = Shops.CategoryID WHERE Categories.Type = 'Small' AND Shops.HasStory='YES' HAVING distance <= 12250 ORDER BY distance ASC LIMIT 0, 20");
}
