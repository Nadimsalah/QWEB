...
$UserID = $_POST["UserID"];
   ...
   $sql="INSERT INTO PoepleInrest (UserID,CategoryId) VALUES ('$UserID','$CategoryID');";
...
if($SearchWord==""){
$res = mysqli_query($con,"SELECT *, (6372.797 * acos(cos(radians($UserLat)) * cos(radians(ShopLat)) * cos(radians(ShopLongt) - radians($UserLongt)) + sin(radians($UserLat)) * sin(radians(ShopLat)))) AS distance FROM Shops WHERE CategoryID ='$CategoryID' AND Type='Our' AND Shops.Status = 'ACTIVE' HAVING distance <= 10000 ORDER BY priority DESC , distance ASC LIMIT $Page, 20");
}else{
$res = mysqli_query($con,"SELECT *, (6372.797 * acos(cos(radians($UserLat)) * cos(radians(ShopLat)) * cos(radians(ShopLongt) - radians($UserLongt)) + sin(radians($UserLat)) * sin(radians(ShopLat)))) AS distance FROM Shops WHERE CategoryID ='$CategoryID' AND Type='Our' AND Shops.Status = 'ACTIVE' AND ShopName LIKE '%$SearchWord%' HAVING distance <= 10000 ORDER BY priority DESC , distance ASC LIMIT $Page, 20");    
}
...
$res2 = mysqli_query($con,"SELECT *, (6372.797 * acos(cos(radians($UserLat)) * cos(radians(ShopLat)) * cos(radians(ShopLongt) - radians($UserLongt)) + sin(radians($UserLat)) * sin(radians(ShopLat)))) AS distance FROM Shops WHERE CategoryID ='$CategoryID' AND Type='Our' HAVING distance <= 10000 ORDER BY priority DESC , distance ASC"); 
