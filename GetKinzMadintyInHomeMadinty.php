...
if($KeyWord==""){
$res = mysqli_query($con,"SELECT *, (6372.797 * acos(cos(radians($UserLat)) * cos(radians(ShopLat)) * cos(radians(ShopLongt) - radians($UserLongt)) + sin(radians($UserLat)) * sin(radians(ShopLat)))) AS distance FROM Shops JOIN Categories ON Categories.CategoryId = Shops.CategoryID WHERE Categories.Type = 'Small' HAVING distance <= 50 ORDER BY distance ASC LIMIT $Page, 20");
}else{
$res = mysqli_query($con,"SELECT *, (6372.797 * acos(cos(radians($UserLat)) * cos(radians(ShopLat)) * cos(radians(ShopLongt) - radians($UserLongt)) + sin(radians($UserLat)) * sin(radians(ShopLat)))) AS distance FROM Shops JOIN Categories ON Categories.CategoryId = Shops.CategoryID WHERE Categories.Type = 'Small' AND Shops.ShopName LIKE '%$KeyWord%' HAVING distance <= 50 ORDER BY distance ASC LIMIT $Page, 20");
}
