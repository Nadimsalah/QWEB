...
$res = mysqli_query($con,"SELECT * FROM Orders LEFT JOIN Users ON Users.UserID = Orders.UserID LEFT JOIN Shops ON Orders.DestinationName = Shops.ShopName WHERE DelvryId='$DelvryId' AND (OrderState = 'waiting' OR OrderState = 'Doing') ORDER BY OrderID DESC");
...
if($row["OrderTypeSender"]=="COMPANY"){
    ...
    $row["AppType"] = "QOON";
    $row["UserCityName"] = "";
    $row["DestnationCityName"] = "";
    $row["UserCountryName"] = "";
    $row["DestnationCountryName"] = "";
    $row["WeightsId"] = "";
...
echo json_encode(array('status_code' => $status_code,'success' => $success ,"data"=>$result[0],"message"=>$message));
    ...
    $status_code = 200;
	$result = ""; 
   echo json_encode(array('status_code' => $status_code,'success' => $success ,"data"=>null,"message"=>$message));
}
die;
mysqli_close($con);
?>