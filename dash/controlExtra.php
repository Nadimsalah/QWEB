...
// Fetch Product Details for Header Context
$prodName = "Extra Categories";
if ($prodid) {
    if ($stmt = $con->prepare("SELECT FoodName FROM Foods WHERE FoodID = ?")) {
        $stmt->bind_param("s", $prodid);
        $stmt->execute();
        $resSn = $stmt->get_result();
        if ($rowSn = $resSn->fetch_assoc()) {
            $prodName = htmlspecialchars($rowSn['FoodName']) . " - Extras";
        }
    }
}
?>
...
<head>
    ...
    <title>Manage Product Extras | QOON</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Inter', sans-serif; }
        ...
        .input-group input[type="text"] { flex: 1; padding: 16px 20px; border-radius: 12px; border: 1px solid var(--border-color); background: var(--bg-app); font-size: 15px; font-weight: 500; outline: none; transition: all 0.2s; }
        .input-group input[type="text"]:focus { border-color: var(--accent-purple); background: #FFF; box-shadow: 0 0 0 4px var(--accent-purple-light); }
        
        .submit-btn { background: var(--accent-purple); color: #FFF; border: none; padding: 0 25px; border-radius: 12px; font-weight: 700; font-size: 14px; cursor: pointer; transition: 0.2s; box-shadow: 0 4px 15px rgba(98, 60, 234, 0.3); }
        .submit-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(98, 60, 234, 0.4); }
        ...
        .cat-item:hover { background: #FFF; border-color: var(--accent-purple); box-shadow: var(--shadow-float); }
        
        .cat-name { font-size: 15px; font-weight: 700; color: var(--text-dark); display: flex; align-items: center; gap: 12px; }
        .cat-name i { color: var(--text-gray); font-size: 18px; }

        .action-tray { display: flex; gap: 10px; }
        .btn-action { padding: 8px 16px; border-radius: 8px; font-weight: 700; font-size: 13px; cursor: pointer; text-decoration: none; transition: 0.2s; display: inline-flex; align-items: center; gap: 6px; }
        .btn-edit { background: rgba(0, 122, 255, 0.1); color: var(--accent-blue); }
            ...
            <div class="flex-grid">
                ...
                <div class="glass-panel">
                    <div class="panel-title">
                        <i class="fas fa-cubes" style="color: var(--accent-purple);"></i> Add Extra Category
                    </div>

                    <form action="AddCatExtraApi.php" method="POST">
                        <div class="input-group">
                            <input type="text" placeholder="e.g. Choose Size, Add Toppings..." name="ExtraCategoryName" required>
                    ...
                    <div class="cat-list">
                        <?php 
                        $resCount = 0;
                        if ($stmt = $con->prepare("SELECT * FROM ExtraCategory WHERE ProductID=?")) {
                            $stmt->bind_param("s", $prodid);
                            $stmt->execute();
                            $res = $stmt->get_result();
                        ...
                        ?>
                            <div class="cat-item">
                                <div class="cat-name">
                                    ...
                                    <?= htmlspecialchars($row["ExtraCategoryName"]) ?>
                                </div>
                                <div class="action-tray">
                                    <a href="ControlExtraValues.php?id=<?= $row["ExtraCategoryID"] ?>&prodid=<?= urlencode($prodid) ?>" class="btn-action btn-edit">
                                        <i class="fas fa-cog"></i> Configure Values
                                    </a>
                                    <a href="DeleteCatExtraShopApi.php?id=<?= $row["ExtraCategoryID"] ?>&prodid=<?= urlencode($prodid) ?>" class="btn-action btn-delete">
                        ...
                        if ($resCount === 0) {
                            echo '<div class="empty-state"><i class="fas fa-inbox fa-3x" style="opacity:0.3; margin-bottom:15px; display:block;"></i>No extras assigned to this product. Add a category group above!</div>';
                        }
                        ?>          
                    </div>
                </div>
                ...
                <div class="glass-panel" style="display: flex; flex-direction: column; justify-content: center; align-items: center; background: linear-gradient(135deg, var(--accent-purple), #4A2BBF); color: #FFF; text-align: center;">
                    <i class="fas fa-tasks fa-5x" style="opacity: 0.2; margin-bottom: 30px;"></i>
                    <h2 style="font-size: 24px; font-weight: 800; margin-bottom: 15px;">Product Extras</h2>
                    <p style="font-size: 14px; font-weight: 500; opacity: 0.8; max-width: 80%; line-height: 1.6;">Create sub-groups of extras (e.g., 'Sauces' or 'Sizes'). You can then add specific price modifiers directly inside each group via the Configure button.</p>
                </div>
