...
$resCfg = mysqli_query($con, "SELECT disUser FROM OrdersJiblerpercentage");
$cfgRow = mysqli_fetch_assoc($resCfg);
$disUser = $cfgRow["disUser"] ?? 0;
...
$resTx = mysqli_query($con, "
    SELECT FeesTransaction.FeesTransactionID,
           FeesTransaction.OrderID,
           FeesTransaction.Money,
           FeesTransaction.CreatedAtFeesTransaction,
           Users.UserID, Users.name, Users.UserPhoto
    FROM FeesTransaction
    JOIN Users ON FeesTransaction.UserID = Users.UserID
    WHERE $where
    ORDER BY FeesTransactionID DESC
...
$resChart = mysqli_query($con, "SELECT MONTH(CreatedAtFeesTransaction) AS Month, COUNT(*) AS Total
    FROM FeesTransaction
    WHERE YEAR(CreatedAtFeesTransaction) = '$currentYear'
    GROUP BY MONTH(CreatedAtFeesTransaction)");
while ($row = mysqli_fetch_assoc($resChart)) {
    $months[(int)$row["Month"]] = (int)$row["Total"];
}
$chartData = implode(',', array_values($months));
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Service Commission | QOON</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Inter', sans-serif; }
        ...
        .kpi-master { display: flex; align-items: center; justify-content: space-between; background: linear-gradient(135deg, #D97706, #F59E0B); border-radius: 20px; padding: 30px 40px; color: #FFF; margin-bottom: 30px; box-shadow: var(--shadow-float); flex-shrink: 0; }
        .kpi-master h4 { font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; opacity: 0.85; margin-bottom: 5px; }
        .kpi-master h1 { font-size: 42px; font-weight: 800; display: flex; align-items: baseline; gap: 10px; }
        .kpi-master h1 span { font-size: 20px; font-weight: 700; opacity: 0.75; }
        .kpi-badge { background: rgba(255,255,255,0.2); border-radius: 12px; padding: 12px 20px; text-align: center; }
        .kpi-badge small { display: block; font-size: 11px; opacity: 0.8; text-transform: uppercase; letter-spacing: 0.8px; margin-bottom: 4px; }
        ...
        .badge-order { font-size: 12px; font-weight: 800; background: rgba(0,122,255,0.1); color: var(--accent-blue); padding: 4px 10px; border-radius: 8px; }
        .rev-amt { font-size: 15px; font-weight: 800; color: var(--accent-orange); }
        ...
        @media (max-width: 600px) {
            ...
            table tbody tr td:nth-child(2) { display: none; }
        }
    </style>
</head>
<body>
    ...
    <main class="main-panel">
        <header class="header">
            <div class="breadcrumb">
                ...
                <span>/</span>
                <span style="color: var(--accent-purple);">Service Commission</span>
            </div>
        </header>

        <div class="kpi-master">
            <div>
                <h4>Total Service Commissions Collected</h4>
                <h1><?= number_format($ServComm, 2) ?> <span>MAD</span></h1>
            </div>
            <div style="display:flex; gap:12px; flex-wrap:wrap;">
                <div class="kpi-badge">
                    <small>Service Fee Rate</small>
                    <strong><?= $disUser ?>%</strong>
                </div>
                <div class="kpi-badge">
                    <small>Share of Total Income</small>
                    <strong><?= $TotalIncome > 0 ? number_format(($ServComm / $TotalIncome) * 100, 1) : '0' ?>%</strong>
                </div>
            </div>
        </div>
        ...
        <div class="rev-channels">
            ...
            <a href="walletEradRased3.php" class="channel-card active">
            ...
            <div class="table-container">
                <div class="table-head">
                    <h2 style="font-size:18px; font-weight:800;">
                        <i class="fas fa-percentage" style="color:var(--accent-purple);"></i> Service Commission Log
                    </h2>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr><td colspan="4" style="text-align:center; padding:30px; color:var(--text-gray);">No service commission logs found.</td></tr>
                        <?php endif; ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="order-detail.php?OrderID=<?= $t['OrderID'] ?>" class="badge-order">#<?= $t['OrderID'] ?></a>
                            </td>
                            <td><span class="rev-amt"><?= number_format($t['Money'], 2) ?> MAD</span></td>
                            <td style="color:var(--text-gray); font-size:13px;"><?= $t['CreatedAtFeesTransaction'] ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                <div class="pagination">
                    <span>Service Commission Records</span>
        ...
        <div class="stats-row" style="margin-bottom:20px;">
            <div class="stat-item">
                <span>Total Service Commission</span>
                <strong><?= number_format($ServComm, 2) ?> MAD</strong>
            </div>
            <div class="stat-item">
                <span>Service Fee Rate</span>
                <strong><?= $disUser ?>% per order</strong>
            </div>
            <div class="stat-item">
